...
static float nucMat[127][127];
void init_pairscore(float matchscore, float mismatchPenalty)
{
	for (int i=0; i<127; i++)
	{
		for (int j=0; j<127; j++)
...
//				}
	...
	nucMat['N']['N']=nucMat['n']['N']=nucMat['N']['n']=0.0;
  ...
  //slightly less wild cards
  nucMat['&']['A']=nucMat['&']['a']=nucMat['A']['&']=nucMat['a']['&']=0.7;
	nucMat['&']['C']=nucMat['&']['c']=nucMat['C']['&']=nucMat['c']['&']=0.7;
	nucMat['&']['T']=nucMat['&']['t']=nucMat['T']['&']=nucMat['t']['&']=0.7;
	nucMat['&']['G']=nucMat['&']['g']=nucMat['G']['&']=nucMat['g']['&']=0.7;
...
//	nucMat['$']['G']=nucMat['$']['g']=nucMat['G']['$']=nucMat['g']['$']=0;
	...
	nucMat['.']['A']=nucMat['.']['a']=nucMat['A']['.']=nucMat['a']['.']=-20.0;
	nucMat['.']['C']=nucMat['.']['c']=nucMat['C']['.']=nucMat['c']['.']=-20.0;
	nucMat['.']['T']=nucMat['.']['t']=nucMat['T']['.']=nucMat['t']['.']=-20.0;
	nucMat['.']['G']=nucMat['.']['g']=nucMat['G']['.']=nucMat['g']['.']=-20.0;

	nucMat['N']['A']=nucMat['N']['a']=nucMat['A']['N']=nucMat['a']['N']=-3.0;
	nucMat['N']['C']=nucMat['N']['c']=nucMat['C']['N']=nucMat['c']['N']=-3.0;
	nucMat['N']['T']=nucMat['N']['t']=nucMat['T']['N']=nucMat['t']['N']=-3.0;
	nucMat['N']['G']=nucMat['N']['g']=nucMat['G']['N']=nucMat['g']['N']=-3.0;

	//for easy alignment to a standard with gaps
	nucMat['X']['A']=nucMat['X']['a']=nucMat['A']['X']=nucMat['a']['X'] = -6.0;
	nucMat['X']['C']=nucMat['X']['c']=nucMat['C']['X']=nucMat['c']['X'] = -6.0;
	nucMat['X']['T']=nucMat['X']['t']=nucMat['T']['X']=nucMat['t']['X'] = -6.0;
	nucMat['X']['G']=nucMat['X']['g']=nucMat['G']['X']=nucMat['g']['X'] = -6.0;
	nucMat['X']['R']=nucMat['X']['r']=nucMat['R']['X']=nucMat['r']['X'] = -6.0;
	nucMat['X']['Y']=nucMat['X']['y']=nucMat['Y']['X']=nucMat['y']['X'] = -6.0;
	nucMat['X']['K']=nucMat['X']['k']=nucMat['K']['X']=nucMat['k']['X'] = -6.0;
	nucMat['X']['M']=nucMat['X']['m']=nucMat['M']['X']=nucMat['m']['X'] = -6.0;
	nucMat['X']['S']=nucMat['X']['s']=nucMat['S']['X']=nucMat['s']['X'] = -6.0;
	nucMat['X']['W']=nucMat['X']['w']=nucMat['W']['X']=nucMat['w']['X'] = -6.0;
	nucMat['X']['B']=nucMat['X']['b']=nucMat['B']['X']=nucMat['b']['X'] = -6.0;
	nucMat['X']['D']=nucMat['X']['d']=nucMat['D']['X']=nucMat['d']['X'] = -6.0;
	nucMat['X']['H']=nucMat['X']['h']=nucMat['H']['X']=nucMat['h']['X'] = -6.0;
	nucMat['X']['V']=nucMat['X']['v']=nucMat['V']['X']=nucMat['v']['X'] = -6.0;
	nucMat['X']['-']=nucMat['X']['-']=3.0;
}


void init_pairscore_aa(float matchscore, float mismatchPenalty)
{
	for (int i=0; i<127; i++)
	{
		for (int j=0; j<127; j++)
	...
	nucMat['Z']['Z']=nucMat['z']['Z']=nucMat['Z']['z']=0.0;
    nucMat['X']['-']=nucMat['-']['X']=matchscore;
}



extern float pairscore(char a, char b)
{
	return nucMat[a][b];
}
...
int align(string* seqa, string* seqb, string* newseqa, string* newseqb, int gip, int gep)
{
	...
	float *oldSS=new float[N+1];
	int **piSS = new int*[M+1];
	int **pjSS = new int*[M+1];
	float *PP = new float[N+1];


	float u = float(-gip); // affine gap initiation penalty
	float v = float(-gep); // affine gap extension penalty
	...
	for (i=1; i < M+1; i++)
	{
		...
		for (j = 1; j < N + 1; j++)
		{
			...
			float tmp_pp = PP[j];
			float pscore = oldSS[j - 1] + pairscore((*seqa)[i - 1], (*seqb)[j - 1]);
...
extern "C" VALUE align_it(VALUE self, VALUE standard, VALUE seq, VALUE gap_init_penalty, VALUE gap_extend_penalty)
{
    ...
    string* seqa = new string(StringValuePtr(standard));  //or StringValuePtr(RSTRING(standard))
    string* seqb = new string(StringValuePtr(seq));
    trim(seqa);
	trim(seqb);
    degap(seqa);
	degap(seqb);
...
extern "C" VALUE align_it_aa(VALUE self, VALUE standard, VALUE seq, VALUE gap_init_penalty, VALUE gap_extend_penalty)
{
    ...
    string* seqa = new string(StringValuePtr(standard));
    string* seqb = new string(StringValuePtr(seq));
    trim(seqa);
	trim(seqb);
    degap(seqa);
...
extern "C" void Init_alignment_ext()
{
